...
def find_best_reference_alignment(main_bits, reference_bits):
    """Find the structural alignment of a reference inside the main bitstream.

    Exact full-reference matches are handled before this function is called.
    For approximate matches, prefer an alignment supported by several exact
    byte-sequence anchors from across the reference, then score the complete
    aligned reference. This avoids a common failure mode of pure global
    correlation where long/repeated zero regions can win even though distinctive
    bytes at the beginning and elsewhere clearly belong to another alignment.

    The returned byte indices are highlighted only when every represented bit
    in that byte is equal at the selected alignment.
    """
    ...
    def evaluate(bit_offset):
        ...
        for byte_index, bit_start in enumerate(range(0, ref_len, 8)):
            ...
            byte_equal = bool(np.all(equal_bits[bit_start:bit_end]))
            if byte_equal:
                ...
                current_run = 0
        return matching_bits, tuple(matching_bytes), longest_run

    # Byte-aligned files are the normal BIN/DAT case and also cover textual
    # references whose logical bit count is a whole number of bytes. Use exact
    # anchors distributed over the whole reference to vote for its real offset.
    if main_len % 8 == 0 and ref_len % 8 == 0 and ref_len >= 8:
        main_bytes = np.packbits(main_bits, bitorder="big").tobytes()
        ref_bytes = np.packbits(reference_bits, bitorder="big").tobytes()
        ref_byte_len = len(ref_bytes)

        # Four-byte anchors are specific enough for ordinary binary data while
        # still surviving isolated byte errors. Sample up to 48 positions across
        # the complete reference so one mismatch cannot determine the alignment.
        seed_len = min(4, ref_byte_len)
        max_seed_start = ref_byte_len - seed_len
        if max_seed_start >= 0:
            if max_seed_start == 0:
                seed_starts = [0]
            else:
                count = min(48, max_seed_start + 1)
                seed_starts = sorted(set(
                    int(round(i * max_seed_start / max(1, count - 1)))
                    for i in range(count)
                ))

            votes = {}
            for seed_start in seed_starts:
                seed = ref_bytes[seed_start:seed_start + seed_len]
                search_from = 0
                # Cap pathological repeated seeds; other distributed anchors
                # still contribute and prevent huge all-zero regions dominating.
                occurrences_for_seed = 0
                while occurrences_for_seed < 256:
                    found = main_bytes.find(seed, search_from)
                    if found < 0:
                        break
                    candidate = found - seed_start
                    if 0 <= candidate <= len(main_bytes) - ref_byte_len:
                        votes[candidate] = votes.get(candidate, 0) + 1
                    search_from = found + 1
                    occurrences_for_seed += 1

            if votes:
                # First use independent anchor support. For tied candidates,
                # prefer coherent exact-byte runs, then total equal bytes/bits.
                max_votes = max(votes.values())
                candidates = [off for off, count in votes.items() if count == max_votes]
                best = None
                for byte_offset in candidates:
                    bit_offset = byte_offset * 8
                    matching_bits, matching_bytes, longest_run = evaluate(bit_offset)
                    rank = (longest_run, len(matching_bytes), matching_bits, -byte_offset)
                    if best is None or rank > best[0]:
                        best = (rank, bit_offset, matching_bits, matching_bytes)
                if best is not None:
                    return best[1], best[2], best[3]

    # Fallback for genuinely bit-shifted/non-byte-aligned streams. Correlation is
    # retained only when no reliable byte-anchor alignment can be established.
    main_signed = main_bits.astype(np.int8) * 2 - 1
    ref_signed = reference_bits.astype(np.int8) * 2 - 1
    scores = correlate(
        main_signed.astype(np.float64),
        ref_signed.astype(np.float64),
        mode="valid",
        method="fft",
    )
    scores = np.rint(scores).astype(np.int64)
    best_offset = int(np.argmax(scores))
    matching_bits, matching_bytes, _ = evaluate(best_offset)
