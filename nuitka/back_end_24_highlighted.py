...
def find_best_reference_alignment(main_bits, reference_bits):
    """Return the best full-length bit alignment and exactly matching reference bytes.

    This is deliberately separate from exact occurrence detection. It lets the
    appendix visualize a near-match even when one or more bits prevent the
    complete reference from being an exact occurrence.
    """
    main_bits = np.asarray(main_bits, dtype=np.uint8).ravel()
    reference_bits = np.asarray(reference_bits, dtype=np.uint8).ravel()
    ref_len = int(reference_bits.size)
    if ref_len == 0 or ref_len > int(main_bits.size):
        return None, 0, ()

    # For values encoded as -1/+1, correlation = matches - mismatches.
    # FFT correlation keeps this practical for long references and inputs.
    main_signed = main_bits.astype(np.float64) * 2.0 - 1.0
    ref_signed = reference_bits.astype(np.float64) * 2.0 - 1.0
    scores = correlate(main_signed, ref_signed, mode="valid", method="fft")
    best_offset = int(np.argmax(scores))

    aligned = main_bits[best_offset:best_offset + ref_len]
    equal_bits = aligned == reference_bits
    matching_bits = int(np.count_nonzero(equal_bits))

    matching_bytes = []
    for byte_index, start in enumerate(range(0, ref_len, 8)):
        end = min(start + 8, ref_len)
        # A displayed reference byte is highlighted only when every source bit
        # represented by that byte agrees at the best alignment.
        if bool(np.all(equal_bits[start:end])):
