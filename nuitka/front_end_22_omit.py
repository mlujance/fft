...
class AnalysisWorker(QObject):
    ...
    def __init__(self, input_file, output_dir, output_name, references, search_references, fs, cwt_wavelet, dwt_wavelet, hex_pattern, skip_bytes, extract_bytes, byte_order, omit_occurrences, omit_from, export_binary, export_binary_name):
        super().__init__()
        ...
        self.omit_occurrences = omit_occurrences
        self.omit_from = omit_from
    ...
    def run(self):
        ...
        try:
            import back_end_24 as analyzer
            ...
            if self.hex_pattern:
                ...
                pattern_result = analyzer.search_hex_pattern(
                    ...
                    omit_occurrences=self.omit_occurrences,
                    omit_from=self.omit_from,
                )
...
class AnalyzerWindow(QMainWindow):
    def __init__(self):
        ...
        self.omit_occurrences_spin = QSpinBox()
        self.omit_occurrences_spin.setRange(0, 100000000)
        self.omit_occurrences_spin.setValue(0)
        self.omit_occurrences_spin.setSuffix(" matches")
        self.omit_occurrences_spin.setToolTip(
            "Ignore this many pattern occurrences before extraction/highlighting/export."
        )

        self.omit_from_combo = QComboBox()
        self.omit_from_combo.addItem("From beginning", "beginning")
        self.omit_from_combo.addItem("From end", "end")
        self.omit_from_combo.setToolTip(
            "Choose whether the omitted occurrences are counted from the start or the end of the file."
        )

        hex_grid.addWidget(self.field_label("Occurrences to omit"), 2, 0)
        hex_grid.addWidget(self.field_label("Omit from"), 2, 1)
        hex_grid.addWidget(self.omit_occurrences_spin, 3, 0)
        hex_grid.addWidget(self.omit_from_combo, 3, 1)
    ...
    def start_analysis(self):
        ...
        self.worker = AnalysisWorker(
            ...
            omit_occurrences=self.omit_occurrences_spin.value(),
            omit_from=self.omit_from_combo.currentData(),
