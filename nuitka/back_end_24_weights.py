...
def load_bits(path: str) -> np.ndarray:
    """Load supported inputs into one canonical MSB-first logical bit stream.

    TXT/CSV files are treated as textual 0/1 streams. Every other supported
    format (BIN, DAT and arbitrary binary files) is read as raw bytes and
    expanded MSB-first. Keeping this conversion in one place guarantees that
    reference highlighting behaves identically across file formats.
    """
    ...
    if ext in (".csv", ".txt"):
        with open(path, "r", encoding="utf-8", errors="ignore") as f:
            text = f.read()
        bits = np.fromiter(
            (1 if c == "1" else 0 for c in text if c in ("0", "1")),
            dtype=np.uint8,
        )
    else:
        ...
        if not raw:
            ...
            # Explicit bitorder avoids format/platform ambiguity.
            bits = np.unpackbits(
...
# ============================================================
# Hexadecimal representation and literal byte-pattern search
# ============================================================
def bits_to_bytes(bits: np.ndarray) -> bytes:
    """Pack normalized logical bits into bytes in stream order."""
...
def parse_hex_pattern(value: str) -> bytes:
    """Parse a literal hexadecimal byte pattern such as 'BB' or 'AA 55 FF'."""
    import re
    cleaned = re.sub(r"0[xX]", "", value or "")
    cleaned = re.sub(r"[\s:_-]+", "", cleaned)
    if not cleaned or len(cleaned) % 2 or not re.fullmatch(r"[0-9A-Fa-f]+", cleaned):
        raise ValueError("Hex pattern must contain complete hexadecimal bytes, for example BB or AA 55 FF.")
    return bytes.fromhex(cleaned)


def search_hex_pattern(bits, pattern_hex, skip_bytes=0, extract_bytes=1, byte_order="big"):
    """Find overlapping literal byte-pattern matches and extract bytes after each match."""
    data = bits_to_bytes(bits)
    pattern = parse_hex_pattern(pattern_hex)
    skip_bytes = int(skip_bytes)
    extract_bytes = int(extract_bytes)
    if skip_bytes < 0:
        raise ValueError("Skip bytes cannot be negative.")
    if extract_bytes < 1:
        raise ValueError("Extract bytes must be at least 1.")
    ...
    while True:
        ...
        if pos < 0:
            break
        extract_start = pos + len(pattern) + skip_bytes
        extract_end = extract_start + extract_bytes
        extracted_raw = data[extract_start:extract_end]
        complete = len(extracted_raw) == extract_bytes
        # Big Endian preserves stream byte order. Little Endian performs a byte swap
        # across the extracted word so the hexadecimal and binary representations
        # themselves visibly reflect the selected byte order.
        extracted = extracted_raw if byte_order == "big" else extracted_raw[::-1]
        occurrences.append({
            "index": len(occurrences) + 1,
            "byte_offset": pos,
            "bit_offset": pos * 8,
            "extract_byte_offset": extract_start,
            "extract_bit_offset": extract_start * 8,
            "extracted_raw": extracted_raw,
            "extracted": extracted,
            "extracted_hex": extracted.hex(" ").upper(),
            "extracted_binary": " ".join(f"{byte:08b}" for byte in extracted),
            "complete": complete,
        })
        start = pos + 1  # preserve overlapping byte-pattern matches

    previous = None
    for item in occurrences:
        if previous is None:
            item["separation_bytes"] = None
            item["separation_bits"] = None
        else:
            delta = item["byte_offset"] - previous
            item["separation_bytes"] = delta
            item["separation_bits"] = delta * 8
        previous = item["byte_offset"]
...
def export_extracted_binary(pattern_result, output_path):
    """Write all extracted blocks consecutively using the selected byte order."""
    if not pattern_result:
        raise ValueError("No hexadecimal pattern result is available for binary export.")
    payload = b"".join(item.get("extracted", b"") for item in pattern_result.get("occurrences", []))
    output_path = os.path.abspath(output_path)
    os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)
    with open(output_path, "wb") as handle:
        handle.write(payload)
    return output_path, len(payload)


def _hex_dump_rows(bits, pattern_result=None, bytes_per_line=16):
    """Build conventional 16-byte hex-dump rows with optional byte highlighting."""
    data = bits_to_bytes(bits)
    pattern_positions = set()
    extracted_positions = set()

    if pattern_result:
        pattern_len = int(pattern_result.get("pattern_bytes", 0))
        extract_len = int(pattern_result.get("extract_bytes", 0))
        for item in pattern_result.get("occurrences", []):
            match_start = int(item["byte_offset"])
            pattern_positions.update(range(match_start, match_start + pattern_len))
            extract_start = int(item["extract_byte_offset"])
            extracted_positions.update(range(extract_start, extract_start + extract_len))
    ...
    for offset in range(0, len(data), bytes_per_line):
        ...
        for column in range(bytes_per_line):
            ...
            if column >= len(chunk):
                cells.append("")
                continue
            value = f"{chunk[column]:02X}"
            # Pattern takes visual priority if an unusual overlap occurs.
            if absolute in pattern_positions:
                value = f'<font color="#B42318"><u><b>{value}</b></u></font>'
            elif absolute in extracted_positions:
                value = f'<font color="#067647"><b>{value}</b></font>'
            cells.append(value)
        rows.append((offset, cells))
    return rows


def _hex_analysis_flowables(main_bits, pattern_result=None):
    if main_bits is None:
        return []
    styles = build_styles()
    byte_style = ParagraphStyle(
        name="HexDumpByte", parent=styles["CustomBody"], fontName="Courier",
        fontSize=7.0, leading=8.2, alignment=1, leftIndent=0, rightIndent=0,
    )
    offset_style = ParagraphStyle(
        name="HexDumpOffset", parent=byte_style, fontName="Courier-Bold", alignment=0,
        textColor=REPORT_THEME["muted_color"],
    )
    out = [
        PageBreak(),
        Paragraph("Hexadecimal Representation", styles["CustomTitle"]),
        Paragraph(
            "Conventional 16-byte rows are used so offsets progress predictably: 00000000, 00000010, "
            "00000020, etc. Offsets are zero-based byte positions.", styles["CustomBody"]),
        Spacer(1, 0.12 * cm),
    ]

    if pattern_result:
        out.append(Paragraph(
            '<font color="#B42318"><b><u>Pattern bytes</u></b></font> &nbsp;&nbsp; '
            '<font color="#067647"><b>Extracted bytes</b></font>',
            styles["CustomBody"],
        ))
        out.append(Spacer(1, 0.10 * cm))

    rows = [[Paragraph("Offset", offset_style)] + [Paragraph(f"{i:02X}", offset_style) for i in range(16)]]
    for offset, cells in _hex_dump_rows(main_bits, pattern_result=pattern_result, bytes_per_line=16):
        rows.append([Paragraph(f"{offset:08X}", offset_style)] + [Paragraph(cell, byte_style) if cell else "" for cell in cells])

    if len(rows) > 1:
        # 1.9 + 16*0.98125 = 17.6 cm: exactly the report's usable width.
        dump_table = Table(rows, colWidths=[1.9 * cm] + [0.98125 * cm] * 16, repeatRows=1, hAlign="LEFT")
        dump_table.setStyle(TableStyle([
            ("BACKGROUND", (0,0), (-1,0), REPORT_THEME["background_light"]),
            ("LINEBELOW", (0,0), (-1,0), 0.45, colors.HexColor("#B8C4D0")),
            ("LINEAFTER", (0,0), (0,-1), 0.45, colors.HexColor("#D5DDE5")),
        ...
        out.append(dump_table)

    if pattern_result:
        out.extend([
            Spacer(1, 0.35 * cm),
            Paragraph("Hexadecimal Pattern Search", styles["Heading2"]),
            Paragraph(
                f"Pattern: <b>{pattern_result['pattern_hex']}</b> &nbsp; | &nbsp; "
                f"Skip after match: <b>{pattern_result['skip_bytes']} bytes</b> &nbsp; | &nbsp; "
                f"Extract: <b>{pattern_result['extract_bytes']} bytes</b> &nbsp; | &nbsp; "
                f"Extraction byte order: <b>{pattern_result['byte_order'].title()} Endian</b> &nbsp; | &nbsp; "
                f"Occurrences: <b>{len(pattern_result['occurrences'])}</b>",
                styles["CustomBody"],
            ),
            Spacer(1, 0.15 * cm),
        ])
        rows = [["#", "Match byte", "Match bit", "Separation", "Extract byte", "Extracted hex", "Binary"]]
        for item in pattern_result["occurrences"]:
            sep = "—" if item["separation_bytes"] is None else f"{item['separation_bytes']} B / {item['separation_bits']} b"
            extracted = item["extracted_hex"] or "—"
            if not item["complete"]:
                extracted += " (truncated)"
            binary = item["extracted_binary"] or "—"
            rows.append([
                str(item["index"]), str(item["byte_offset"]), str(item["bit_offset"]), sep,
                str(item["extract_byte_offset"]), extracted, binary,
            ])
        if len(rows) == 1:
            out.append(Paragraph("No occurrence of the requested hexadecimal pattern was found.", styles["CustomBody"]))
        else:
            table = Table(rows, colWidths=[0.7*cm, 1.7*cm, 1.7*cm, 2.3*cm, 1.8*cm, 3.8*cm, 5.6*cm], repeatRows=1)
            table.setStyle(TableStyle([
                ("BACKGROUND", (0,0), (-1,0), REPORT_THEME["primary_color"]),
                ("TEXTCOLOR", (0,0), (-1,0), colors.white),
                ("FONTNAME", (0,0), (-1,0), REPORT_THEME["title_font"]),
...
# ============================================================
# Exact bit-level reference matching and PDF appendix
# ============================================================

@dataclass
class ReferenceMatch:
    """Normalized reference bitstream and its exact positions in the main input."""
...
def find_exact_bit_occurrences(main_bits, reference_bits):
    """
    Find every overlapping exact bit-level occurrence.

    Both sequences must already be normalized into logical bits. Returned
    offsets are zero-based BIT positions, not source-file byte offsets.
    """
    ...
    # Filter by the first reference bit, then verify the complete candidate.
    # Advancing candidate-by-candidate preserves overlapping matches.
...
def find_best_reference_alignment(main_bits, reference_bits):
    """Find the most coherent reference alignment, prioritising byte values.

    For byte-aligned data, comparison is deliberately word/byte-first rather
    than zero-density/correlation-first. Every possible byte offset is scored
    by direct equality against the complete reference. Exact non-zero byte
    matches carry substantially more weight than 00 matches, so long zero runs
    cannot pull the alignment away from distinctive values such as 7F FC FF FF.

    Once the byte alignment is selected, highlighting is still exact: a byte is
    highlighted only when every represented bit in that byte matches. A bitwise
    correlation fallback is retained only for genuinely non-byte-aligned data.
    """
    main_bits = np.ascontiguousarray(main_bits, dtype=np.uint8).ravel()
    reference_bits = np.ascontiguousarray(reference_bits, dtype=np.uint8).ravel()
    ref_len = int(reference_bits.size)
    main_len = int(main_bits.size)
    if ref_len == 0 or ref_len > main_len:
        return None, 0, ()

    def evaluate(bit_offset):
        aligned = main_bits[bit_offset:bit_offset + ref_len]
        equal_bits = np.equal(aligned, reference_bits)
        matching_bits = int(np.count_nonzero(equal_bits))
        matching_bytes = []
        for byte_index, bit_start in enumerate(range(0, ref_len, 8)):
            bit_end = min(bit_start + 8, ref_len)
            if bool(np.all(equal_bits[bit_start:bit_end])):
                matching_bytes.append(byte_index)
        return matching_bits, tuple(matching_bytes)

    # Normal BIN/DAT case (and TXT/CSV streams containing a whole number of
    # bytes): compare the reference directly against EVERY possible byte offset.
    # This is cheap for the intended reference sizes and, unlike correlation,
    # cannot reward a candidate merely because both regions contain many zeros.
    if main_len % 8 == 0 and ref_len % 8 == 0:
        main_bytes = np.frombuffer(np.packbits(main_bits, bitorder="big").tobytes(), dtype=np.uint8)
        ref_bytes = np.frombuffer(np.packbits(reference_bits, bitorder="big").tobytes(), dtype=np.uint8)
        n_candidates = int(main_bytes.size - ref_bytes.size + 1)

        if n_candidates > 0:
            # Non-zero values are the informative part of the alignment. 00 is
            # still counted, but at 1/8 of the weight of any non-zero byte.
            # Direct byte mismatches are penalised, especially where the
            # reference itself is non-zero.
            informative = ref_bytes != 0
            weights = np.where(informative, 8, 1).astype(np.int32)
            mismatch_penalty = np.where(informative, 4, 1).astype(np.int32)

            best_rank = None
            best_byte_offset = 0
            for byte_offset in range(n_candidates):
                candidate = main_bytes[byte_offset:byte_offset + ref_bytes.size]
                equal = candidate == ref_bytes

                weighted_matches = int(np.sum(weights[equal], dtype=np.int64))
                weighted_mismatches = int(np.sum(mismatch_penalty[~equal], dtype=np.int64))
                nonzero_matches = int(np.count_nonzero(equal & informative))
                total_byte_matches = int(np.count_nonzero(equal))

                # Strongly reward a coherent equal prefix. This makes a real
                # shared beginning (e.g. 7F FC FF FF ...) beat unrelated zero
                # regions even when later bytes contain a few differences.
                mismatch_idx = np.flatnonzero(~equal)
                prefix_bytes = int(mismatch_idx[0]) if mismatch_idx.size else int(ref_bytes.size)

                # Longest contiguous exact-byte run provides another coherence
                # signal without allowing isolated 00 matches to dominate.
                longest_run = 0
                current_run = 0
                ...
                score = weighted_matches - weighted_mismatches
                rank = (
                    score,
                    nonzero_matches,
                    prefix_bytes,
            ...
            best_offset = best_byte_offset * 8
            matching_bits, matching_bytes = evaluate(best_offset)
            return best_offset, matching_bits, matching_bytes

    # Fallback only for streams that truly require a bit-shifted alignment.
    # Ones/non-zero information is given extra influence so zero-heavy regions
    # do not dominate the selected offset.
    main_float = main_bits.astype(np.float64)
    ref_float = reference_bits.astype(np.float64)
    ones_score = correlate(main_float, ref_float, mode="valid", method="fft")
    zeros_score = correlate(1.0 - main_float, 1.0 - ref_float, mode="valid", method="fft")
    scores = 4.0 * ones_score + zeros_score
...
def search_reference_binaries(main_path, reference_paths):
    """
    Compare references against the main input in the logical BIT domain.

    The same load_bits() function is used for the main input and every
    reference. This makes matching independent of the physical storage format:
    a TXT/CSV sequence and a BIN/DAT sequence can match when they normalize to
    the same ordered bits.
    """
    ...
    for slot, path in enumerate(reference_paths, start=1):
        ...
        if reference_bits.size == 0:
            raise ValueError(
                f"Reference {slot} contains no valid bits."
            )

        offsets = find_exact_bit_occurrences(
            main_bits,
            reference_bits,
        )
        # If at least one exact occurrence exists, use it as the display alignment.
        # This guarantees that an exact reference is rendered as 100% matching,
        # independently of correlation ties in repetitive data.  Only fall back
        # to the best approximate full-length alignment when no exact occurrence
        # exists.
        if offsets:
            best_offset = int(offsets[0])
            matching_bits = int(reference_bits.size)
            matching_byte_indices = tuple(range((int(reference_bits.size) + 7) // 8))
        else:
            best_offset, matching_bits, matching_byte_indices = find_best_reference_alignment(
                main_bits, reference_bits
            )
        ...
        results.append(
            ReferenceMatch(
                ...
                matching_byte_indices=matching_byte_indices,
            )
        )
...
def _reference_bit_string(bits):
    return "".join(
        "1" if int(bit) else "0"
...
def _reference_signal_plot(bits, slot):
    """
    Draw the reference with the same plotting conventions used by the
    original raw binary signal figure.
    """
    path = os.path.join(
        IMG_DIR,
        f"reference_{slot}_signal.png",
    )
    ...
    plt.title(
        f"Reference {slot} Binary Signal "
        f"(complete input: {len(bits):,} bits)"
    )
    plt.xlabel("Sample index")
    plt.ylabel("Bit value")
    plt.ylim(-0.1, 1.1)
...
def _reference_summary_table(match):
    rows = [
        ["Reference", "Length [bits]", "Found", "Occurrences"],
        [
...
def _reference_occurrence_table(match):
    rows = [[
        "Occurrence",
        "Start bit",
        "End bit",
...
def _reference_appendix_flowables(matches):
    """
    Build an appendix with the original report's typography and colour theme.

    The original analysis, figures, metrics and CWT/DWT aesthetics are not
    recalculated here.
    """
    if not matches:
        return []
    ...
    appendix = [
        ...
        Paragraph(
            "Each reference is normalized with the same input loader used for "
            "the primary signal and is then searched as a complete exact bit "
            "sequence. The comparison therefore uses logical bit values rather "
            "than the source file's byte representation. TXT/CSV and BIN/DAT "
            "inputs can match when they normalize to the same ordered bits. "
            "All reported positions are zero-based bit offsets and end "
    ...
    for match_index, match in enumerate(matches):
        ...
        appendix.append(Spacer(1, 0.10 * cm))
        appendix.append(
            _reference_summary_table(match)
        )
        appendix.append(Spacer(1, 0.25 * cm))

        reference_plot = _reference_signal_plot(
            match.bits,
            match.slot,
        )
        ...
        appendix.append(
            Paragraph(
                "Reference Binary Representation",
                styles["Heading3"],
            )
        )
        appendix.append(Spacer(1, 0.08 * cm))
        appendix.append(
            Image(
                reference_plot,
                width=REPORT_THEME["figure_width_cm"] * cm,
                height=REPORT_THEME["figure_width_cm"] * 0.58 * cm,
            )
        )
        appendix.append(Spacer(1, 0.10 * cm))
        appendix.append(
            Paragraph(
                "Complete normalized reference sequence. The representation "
                "uses the same binary plotting conventions as the principal "
                "signal plot.",
        ...
        appendix.append(
            Paragraph(
                "Complete reference sequence shown as a conventional hexadecimal dump. "
                "Each row contains 16 bytes and uses zero-based byte offsets.",
                styles["CustomCaption"],
            )
        )
        appendix.append(Spacer(1, 0.08 * cm))

        if match.best_offset is not None:
            match_pct = (100.0 * match.matching_bits / match.size_bits) if match.size_bits else 0.0
            appendix.append(Paragraph(
                f'<b>Best full-length alignment:</b> main bit {match.best_offset:,} &nbsp; | &nbsp; '
                f'<b>Matching bits:</b> {match.matching_bits:,}/{match.size_bits:,} ({match_pct:.2f}%). '
                'Bytes whose represented bits match at this alignment are highlighted with a '
                '<span backColor="#FFF2A8">yellow background</span>. Text colour is unchanged. '
                'When there is no complete occurrence, this alignment is selected from exact byte anchors across the reference and then verified bit by bit.',
                styles["CustomBody"],
            ))
            appendix.append(Spacer(1, 0.10 * cm))

        ref_byte_style = ParagraphStyle(
            name=f"ReferenceHexByte{match.slot}",
        ...
        ref_offset_style = ParagraphStyle(
            name=f"ReferenceHexOffset{match.slot}",
            parent=ref_byte_style,
            fontName="Courier-Bold",
            alignment=0,
            textColor=REPORT_THEME["muted_color"],
        )

        ref_rows = [[Paragraph("Offset", ref_offset_style)] +
                    [Paragraph(f"{i:02X}", ref_offset_style) for i in range(16)]]
        for ref_offset, ref_cells in _hex_dump_rows(match.bits, bytes_per_line=16):
            ref_rows.append(
                [Paragraph(f"{ref_offset:08X}", ref_offset_style)] +
                [Paragraph(cell, ref_byte_style) if cell else "" for cell in ref_cells]
            )

        if len(ref_rows) > 1:
            # Same 17.6 cm usable width as the main hexadecimal representation.
            ref_dump_table = Table(
                ref_rows,
                colWidths=[1.9 * cm] + [0.98125 * cm] * 16,
                repeatRows=1,
                hAlign="LEFT",
            )
            ref_table_style = [
            ...
            # Column 0 is the offset; byte columns are 1..16. Row 0 is the header.
            for byte_index in match.matching_byte_indices:
                row = 1 + (byte_index // 16)
                col = 1 + (byte_index % 16)
                ref_table_style.append(("BACKGROUND", (col, row), (col, row), colors.HexColor("#FFF2A8")))
            ref_dump_table.setStyle(TableStyle(ref_table_style))
            appendix.append(ref_dump_table)
        ...
        if match.found:
            appendix.append(
                _reference_occurrence_table(match)
            )
        else:
            appendix.append(
                Paragraph(
                    "No exact bit-level occurrence was found.",
                    styles["CustomBody"],
                )
            )
...
def build_original_report_with_appendix(
    ...
    """
    Build the established original PDF report and add the optional appendix.

    build_pdf(), generate_plots(), the original metrics, figure styling,
    CWT colormap and report layout remain unchanged.
    """
    appendix = _reference_appendix_flowables(matches)
    hex_section = _hex_analysis_flowables(main_bits, pattern_result=pattern_result)
    extra_flowables = hex_section + appendix

    if not extra_flowables:
        build_pdf(
            metrics,
            paths,
    ...
    SimpleDocTemplate.build = build_with_appendix
