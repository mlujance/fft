...
def search_hex_pattern(
    bits, pattern_hex, skip_bytes=0, extract_bytes=1, byte_order="big",
    omit_occurrences=0, omit_from="beginning",
):
    """Find logical matches without double-counting patterns inside a match region.

    After accepting a match, the next search begins after:
    pattern + skipped bytes + extracted bytes.
    Thus a pattern occurring inside skipped/extracted bytes is not a new occurrence.
    """
    data = bits_to_bytes(bits)
    pattern = parse_hex_pattern(pattern_hex)
    skip_bytes = int(skip_bytes)
    extract_bytes = int(extract_bytes)
    omit_occurrences = int(omit_occurrences)

    if skip_bytes < 0:
        raise ValueError("Skip bytes cannot be negative.")
    if extract_bytes < 1:
        raise ValueError("Extract bytes must be at least 1.")
    if omit_occurrences < 0:
        raise ValueError("Occurrences to omit cannot be negative.")
    if byte_order not in ("big", "little"):
        raise ValueError("Byte order must be 'big' or 'little'.")
    if omit_from not in ("beginning", "end"):
        raise ValueError("Omit direction must be 'beginning' or 'end'.")

    logical_positions = []
    search_start = 0
    consumed_span = len(pattern) + skip_bytes + extract_bytes

    while search_start <= len(data) - len(pattern):
        pos = data.find(pattern, search_start)
        if pos < 0:
            break
        logical_positions.append(pos)
        search_start = pos + consumed_span

    total_occurrences = len(logical_positions)
    omitted_count = min(omit_occurrences, total_occurrences)
    if omitted_count == 0:
        retained_positions = logical_positions
    elif omit_from == "beginning":
        retained_positions = logical_positions[omitted_count:]
    else:
        retained_positions = logical_positions[:-omitted_count]

    index_by_position = {p: i for i, p in enumerate(logical_positions, 1)}
    occurrences = []
    for retained_index, pos in enumerate(retained_positions, 1):
        ...
        occurrences.append({
            ...
            "original_index": index_by_position[pos],
            "byte_offset": pos,
            "bit_offset": pos * 8,
    ...
    return {
        ...
        "omit_occurrences": omit_occurrences,
        "omit_from": omit_from,
    ...
    if pattern_result:
        out.extend([
            ...
            Paragraph(
                ...
                f"Found: <b>{pattern_result.get('total_occurrences', len(pattern_result['occurrences']))}</b> &nbsp; | &nbsp; "
                f"Omitted: <b>{pattern_result.get('omitted_count', 0)}</b> "
                f"({'from beginning' if pattern_result.get('omit_from', 'beginning') == 'beginning' else 'from end'}) &nbsp; | &nbsp; "
                f"Processed: <b>{len(pattern_result['occurrences'])}</b>",
                styles["CustomBody"],
            ),
            Spacer(1, 0.15 * cm),
        ])
...
def find_best_reference_alignment(main_bits, reference_bits):
    """Compare the reference primarily by absolute position, byte by byte.

    The report's Reference Hexadecimal Values table is a dump of the reference
    starting at reference offset 0.  Therefore its primary highlight must answer
    the simplest visual question: does reference byte N equal main byte N?

    This avoids a global alignment moving the comparison into a zero-heavy area
    and then making identical bytes at the beginning appear unhighlighted.
    A secondary best alignment is still calculated for diagnostic reporting,
    but it does not replace the direct positional highlight.
    """
    ...
    ref_len = int(reference_bits.size)
    main_len = int(main_bits.size)
    if ref_len == 0 or main_len == 0:
        return None, 0, ()

    # PRIMARY RESULT: exact position-to-position comparison from offset zero.
    # Highlight a reference byte only when every represented bit equals the
    # corresponding main-input bits at the SAME absolute bit position.
    overlap = min(main_len, ref_len)
    direct_equal = np.equal(main_bits[:overlap], reference_bits[:overlap])
    direct_matching_bits = int(np.count_nonzero(direct_equal))
    direct_matching_bytes = []
    for byte_index, bit_start in enumerate(range(0, overlap, 8)):
        bit_end = min(bit_start + 8, overlap)
        # Do not mark a partial trailing byte unless all its represented bits match.
        if bool(np.all(direct_equal[bit_start:bit_end])):
            direct_matching_bytes.append(byte_index)

    # The highlight intentionally uses the direct comparison above.  Offset 0
    # is returned because that is exactly the coordinate system shown in the
    # reference hexadecimal table.  Exact-occurrence searching remains a
    # separate diagnostic in search_reference_binaries().
    return 0, direct_matching_bits, tuple(direct_matching_bytes)
...
def search_reference_binaries(main_path, reference_paths):
    ...
    for slot, path in enumerate(reference_paths, start=1):
        ...
        # The hexadecimal highlight is deliberately position-to-position.
        # Exact occurrences are still reported below, but they do not move the
        # visual comparison away from byte/bit offset zero.
    ...
    for match_index, match in enumerate(matches):
        ...
        if match.best_offset is not None:
            ...
            appendix.append(Paragraph(
                f'<b>Direct positional comparison:</b> reference bit 0 vs main bit 0 &nbsp; | &nbsp; '
                f'<b>Matching bits:</b> {match.matching_bits:,}/{match.size_bits:,} ({match_pct:.2f}%). '
                'Yellow bytes are exact byte-for-byte matches at the same absolute offset in both files. '
                'Zero bytes receive no special weight. Exact occurrences elsewhere in the main input are reported separately below.',
                styles["CustomBody"],
            ))
            appendix.append(Spacer(1, 0.10 * cm))
