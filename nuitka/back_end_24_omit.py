...
def search_hex_pattern(
    ...
    """Find literal byte-pattern matches, optionally omit matches at either end,
    then extract bytes only from the retained occurrences.

    omit_occurrences=3, omit_from="beginning" ignores the first three matches.
    omit_occurrences=3, omit_from="end" ignores the last three matches.
    """
    ...
    # First locate every occurrence. Filtering happens afterwards so "from end"
    # is unambiguous and extraction/export/highlighting all use the same subset.
    all_positions = []
    start = 0
    while True:
    ...
    if omitted_count == 0:
        retained_positions = all_positions
    elif omit_from == "beginning":
        retained_positions = all_positions[omitted_count:]
    else:
        retained_positions = all_positions[:-omitted_count]

    occurrences = []
    for retained_index, pos in enumerate(retained_positions, start=1):
        original_index = all_positions.index(pos) + 1
