...
def find_best_reference_alignment(main_bits, reference_bits):
    """Return the best full-length bit alignment and matching reference bytes.

    Works on the canonical bit arrays produced by load_bits(), so CSV, TXT,
    BIN and DAT references use exactly the same comparison/highlight path.
    Integer convolution is used instead of floating-point FFT correlation;
    this avoids rounding/tie artefacts and makes the selected alignment fully
    deterministic for binary data.
    """
    ...
    # matches = positions where both are 1 + positions where both are 0.
    # np.correlate on long arrays can be expensive, so keep scipy's optimized
    # correlation but use integer-valued inputs and round only the final scores.
    ...
    aligned = main_bits[best_offset:best_offset + ref_len]
    ...
    return best_offset, matching_bits, tuple(matching_bytes)
    ...
    for match_index, match in enumerate(matches):
        ...
        if match.best_offset is not None:
            ...
            appendix.append(Paragraph(
                ...
                'This approximate alignment is shown independently of the exact-occurrence result.',
                styles["CustomBody"],
            ))
            appendix.append(Spacer(1, 0.10 * cm))
