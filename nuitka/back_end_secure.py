...
def cwt_window_energy_summary(signal: np.ndarray, fs: float, wavelet_name: str):
    """Compute compact CWT energy metrics for beginning, middle and end windows."""
    n = len(signal)
    if n == 0:
        return []
    ...
    for i, label in enumerate(labels):
        ...
        # Guarantee at least one sample when possible.
        if end <= start:
            end = min(n, start + 1)
        ...
        coef, freqs, scales = cwt_analysis(
            window,
            fs=fs,
        ...
        dominant_idx = int(np.argmax(scale_energy)) if len(scale_energy) else 0

        summary.append({
            "window": label,
            "start": start,
            "end": end - 1,
            "mean_energy": float(np.mean(power)) if power.size else 0.0,
            "max_energy": float(np.max(power)) if power.size else 0.0,
            "dominant_scale": int(scales[dominant_idx]) if len(scales) else 0,
            "dominant_pseudofreq": (
                float(freqs[dominant_idx]) if len(freqs) else 0.0
            ),
        })
...
def generate_plots(bits: np.ndarray, fs: float, cwt_wavelet: str, dwt_wavelet: str):
    ...
    # 1) Raw signal - complete input bitstream
    n_show = len(bits)
    plt.figure(figsize=(10, 4))
    plt.plot(np.arange(n_show), bits, label="Bits (0/1)", linewidth=0.7)
    plt.title(f"Binary Signal (complete input: {len(bits):,} bits)")
    ...
    lag_show = len(corr)
    plt.figure(figsize=(10, 4))
    plt.plot(np.arange(lag_show), corr, label="Normalized autocorrelation", linewidth=0.7)
    ...
    # Compute mutual information across every valid non-zero lag.
    # For extremely large inputs this can be computationally expensive,
    # but it intentionally covers the complete file as requested.
    max_lag = max(1, len(bits) - 1)
    ...
    plt.title("Run-Length Distribution", fontweight="bold")
    plt.xlabel("Run length [samples]", fontweight="bold")
    plt.ylabel("Count", fontweight="bold")
    plt.grid(True, alpha=0.42, linewidth=1.0)
    ...
    metrics["cwt_window_energy"] = cwt_window_energy_summary(
        signal,
        fs=fs,
    ...
    if len(density_series) >= 200:
        # Use the complete reduced-density series without downsampling.
    ...
    metrics["input_start_bit"] = 0
    metrics["input_end_bit"] = int(len(bits) - 1)
...
def interpret_wavelet_windows(metrics):
    windows = metrics.get("cwt_window_energy", [])
    if not windows:
        return "Windowed CWT energy could not be computed."

    strongest = max(windows, key=lambda x: x["max_energy"])
    weakest = min(windows, key=lambda x: x["mean_energy"])

    return (
        f"The strongest local CWT response is in the {strongest['window']} window "
        f"(max energy {strongest['max_energy']:.6g}), while the lowest mean energy "
        f"is in the {weakest['window']} window ({weakest['mean_energy']:.6g}). "
        "Near-zero energy over a long window is expected for nearly constant bit regions."
    )
...
def add_figure(story, title, image_path, caption, styles, width=None):
    """Add a figure as one compact block to avoid orphan titles/captions."""
    if width is None:
        width = REPORT_THEME["figure_width_cm"]

    image_height = width * 0.50 * cm

    block = [
        Paragraph(title, styles["Heading3"]),
        Spacer(1, 0.06 * cm),
        Image(image_path, width=width * cm, height=image_height),
        Spacer(1, 0.07 * cm),
        Paragraph(caption, styles["CustomCaption"]),
        Spacer(1, 0.16 * cm),
    ]
...
def make_wavelet_window_energy_table(metrics):
    """Build a compact table with CWT energy statistics for three file windows."""
    rows = [[
        "Window",
        "Bit range",
        "Mean energy",
...
def build_pdf(metrics, paths, source_file):
    ...
    add_section(story, "Windowed Wavelet Interpretation", interpret_wavelet_windows(metrics), styles)
    ...
    add_figure(
        ...
        "The complete input bitstream. Long flats indicate persistence; frequent toggling indicates local alternation.",
        styles
    )
    add_figure(
    ...
    longest_runs_block = [
        Paragraph("15 Longest Runs", styles["Heading2"]),
        Paragraph(
        ...
        make_longest_runs_table(metrics),
        Spacer(1, 0.22 * cm),
    ]
    story.append(KeepTogether(longest_runs_block))
    ...
    wavelet_window_block = [
        Paragraph("Wavelet Energy by Window", styles["Heading2"]),
        Paragraph(
            "The file is divided into Beginning, Middle and End. "
            "Very low energy indicates locally constant or weakly varying data; "
            "higher values indicate stronger local changes or oscillatory structure.",
            styles["CustomCaption"],
        ),
        make_wavelet_window_energy_table(metrics),
        Spacer(1, 0.20 * cm),
    ]
    story.append(KeepTogether(wavelet_window_block))
